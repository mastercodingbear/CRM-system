/* eslint-disable @typescript-eslint/naming-convention */
import { ThinqFormFieldUI } from '@core/thinq/thinq.type';

export class Candidate {
  fields: ThinqFormFieldUI[] = [
    {
      fieldName: 'Tag',
      startCol: 1,
      endCol: 9
    },
    {
      fieldName: 'UserId',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'Status',
      startCol: 1,
      endCol: 3
    },
    {
      fieldName: 'StatusDescription',
      startCol: 3,
      endCol: 9
    },
    {
      fieldName: 'Relocate',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'Ethnic',
      startCol: 1,
      endCol: 3
    },
    {
      fieldName: 'Gender',
      startCol: 3,
      endCol: 5
    },
    {
      fieldName: 'FirstName',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'LastName',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'JobTitle',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'CompanyName',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'Nationality',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'Salary',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'CurrentLocation',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'Telephone',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'Email',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'DateOfBirth',
      startCol: 9,
      endCol: 11
    },
    {
      fieldName: 'Age',
      startCol: 11,
      endCol: 13
    },
    {
      fieldName: 'OptOut',
      startCol: 1,
      endCol: 3
    },
    {
      fieldName: 'SuperReferrer',
      startCol: 3,
      endCol: 5
    },
    {
      fieldName: 'LineId',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'IdPassport',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'Industry',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'Qualification',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'YearsExperience',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'LeadSource',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'LeadSourceDescription',
      startCol: 5,
      endCol: 13
    },
    {
      fieldName: 'Description'
    },
    {
      fieldName: 'CV'
    },
    {
      fieldName: 'CvTxtField'
    },
    {
      fieldName: 'Address'
    },
    {
      fieldName: 'OfficePhone',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'OtherPhone',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'HomePhone',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'SearchLastDate',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'SearchLastTerms',
      startCol: 5,
      endCol: 13
    },
    {
      fieldName: 'SearchUser',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'SearchCounter',
      startCol: 5,
      endCol: 13
    },
    {
      fieldName: 'SearchHitTerms'
    },
  ];
  constructor() {
  }
}
