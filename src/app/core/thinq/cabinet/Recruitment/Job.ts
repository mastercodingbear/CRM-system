/* eslint-disable @typescript-eslint/naming-convention */
import { ThinqFormFieldUI } from '@core/thinq/thinq.type';

export class Job {
  fields: ThinqFormFieldUI[] = [
    {
      fieldName: 'Status',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'Published',
      startCol: 4,
      endCol: 7
    },
    {
      fieldName: 'StatusDescription',
      startCol: 7,
      endCol: 13
    },
    {
      fieldName: 'Sector',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'OpenDate',
      startCol: 4,
      endCol: 7
    },
    {
      fieldName: 'CloseDate',
      startCol: 7,
      endCol: 10
    },
    {
      fieldName: 'StartDate',
      startCol: 10,
      endCol: 13
    },
    {
      fieldName: 'JobTitle',
      startCol: 1,
      endCol: 13
    },
    {
      fieldName: 'Contact'
    },
    {
      fieldName: 'Organisation'
    },
    {
      fieldName: 'AssignedTo',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'AdminId',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'EmailJobApplicationTo',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'RemoteId1',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'RemoteId2',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'RemoteId3',
      startCol: 9,
      endCol: 13
    },
    {
      fieldName: 'Ethnic',
      startCol: 1,
      endCol: 5
    },
    {
      fieldName: 'Fees',
      startCol: 5,
      endCol: 9
    },
    {
      fieldName: 'Competitors',
      startCol: 1,
      endCol: 13
    },
    {
      fieldName: 'Description'
    },
    {
      fieldName: 'Salary',
      startCol: 1,
      endCol: 13
    },
    {
      fieldName: 'Currency',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'SalaryFrom',
      startCol: 4,
      endCol: 7
    },
    {
      fieldName: 'SalaryTo',
      startCol: 7,
      endCol: 13
    },
    {
      fieldName: 'Currency',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'SalaryCTCFrom',
      startCol: 4,
      endCol: 7
    },
    {
      fieldName: 'SalaryCTCTo',
      startCol: 7,
      endCol: 13
    },
    {
      fieldName: 'MA',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'MADescription',
      startCol: 4,
      endCol: 13
    },
    {
      fieldName: 'Pension',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'PensionDescription',
      startCol: 4,
      endCol: 13
    },
    {
      fieldName: '13th',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'PerformanceBonus',
      startCol: 4,
      endCol: 7
    },
    {
      fieldName: 'PerformanceDesc',
      startCol: 7,
      endCol: 13
    },
    {
      fieldName: 'SalaryOther'
    },
    {
      fieldName: 'Area',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'Country',
      startCol: 4,
      endCol: 7
    },
    {
      fieldName: 'Suburb',
      startCol: 7,
      endCol: 10
    },
    {
      fieldName: 'Location',
      startCol: 10,
      endCol: 13
    },
    {
      fieldName: 'ImageUploadedId',
      startCol: 1,
      endCol: 4
    },
    {
      fieldName: 'Keywords',
      startCol: 4,
      endCol: 13
    },
    {
      fieldName: 'ShortDescription'
    },
    {
      fieldName: 'Overview'
    },
    {
      fieldName: 'DescriptionOfRole'
    },
    {
      fieldName: 'MinimumRequirements'
    },
    {
      fieldName: 'OriginalJobSpec'
    },
  ];
  constructor() {
  }
}
