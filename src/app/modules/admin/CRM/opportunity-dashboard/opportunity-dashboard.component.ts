...
import { OPPORTUNITY_STATUS, SALES_FUNNEL_STATUS, OPPORTUNITY_STATUS_COLOR } from '@core/config/opportunity.config';
import { Months } from '@core/config/app.config';
...
export class OpportunityDashboardComponent implements OnInit, OnDestroy {
  ...
  /**
   * Get Opportunity win rate
   *
   * @returns active oppor / total oppor
   */
  getWinRate(): number {
  ...
  getChartSeriesByStage(): ApexAxisChartSeries {
    const chartSeries = [];
    for (const status of OPPORTUNITY_STATUS) {
  ...
  private _prepareChartData(): void {
    ...
    this.chartSalesFunnel = {
      series: [
        {
          name: 'basic',
          data: SALES_FUNNEL_STATUS.map(status => this.opportunityByStage[status]?.length ?? 0)
        }
      ],
      chart: {
        type: 'bar',
      ...
      xaxis: {
        categories: SALES_FUNNEL_STATUS
      }
    };
