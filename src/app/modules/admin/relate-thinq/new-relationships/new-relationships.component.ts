...
import { appRelationship } from '@core/config/app.config';
...
export class NewRelationshipsComponent implements OnInit, AfterViewInit {

  @Input() searchRelations: ThinqRelation[];
  @Output() updateRelationEvent: EventEmitter<RelationshipActionParam> = new EventEmitter<RelationshipActionParam>();
  @ViewChild(MatPaginator) private _paginator: MatPaginator;
  @ViewChild(MatSort) private _sort: MatSort;

  relations: ThinqRelation[];
  relationshipOptions = appRelationship;
  ...
  ngOnInit(): void {
    ...
    this.searchInputControl.valueChanges
      .pipe(
        debounceTime(300),
        switchMap((query) => {
          const displayedRelation = this.getRelation(0, this.pagination.size, '', 'asc', query);
          this.pagination = displayedRelation.pagination;
          this.relations = displayedRelation.relations;
          return of(true);
        }),
        map(() => {
  ...
  /**
   * Get thinq list that can be related
   *
   ...
   * @returns pagination and relationships
   */
  getRelation(
    page: number = 0,
    size: number = 10,
  ...
  /**
   * Set relationship with thinq
   *
   * @param relationOption Relationship Id i.e. parentOf, childOf, etc
   * @param targetId Target thinq Id
   */
  setRelation(relationOption: number, targetId: number): void {
